...
from batcher_mio import MioLocalization
import cv2
...
batcher = MioLocalization(path=opt.path,batch_size=30)


def display(image1, mask1, name="hola.png"):
...
def get_sample(discriminator):
    ...
    Y = discriminator(X)
    Y = batcher.get_back(Y.data.numpy())[0]
    im = batcher.draw(X[0].copy(),*Y)
    cv2.imshow('lol',im)
    cv2.waitKey(5000)
...
def visualize():
    ...
    all_hidden = arc._forward(sample)[:, 0, :]  # (2*numGlimpses, controller_out)
    ...
    for i, (mask1) in enumerate(masks1):
        display(sample[0], mask1, "img_{}".format(i))
